...
This article shows the last usage by describing a facility to display a report to its User as soon as it is generated by a program. The way employed to achieve this is to have the program respond to the user’s request by displaying the report as a PDF on the user’s browser. 
...
### Configure DataGate's Renderer Path
...
```cs
    ...
    {
        . . . 
        DataGate.Client.Properties.PrintingProps.Default["Renderer"] = "C:/bin/TheRenderer/Open.Renderer.exe";
        . . .
    }
```

> The Render's path should be externalized to some application setting to facilitate its configuration.

You will have to select one of the three [Renderers available from ASNA](/manuals/hosting/mom/manuscript-renderer.html) or build your own. 

### Enable Controller's Endpoint
